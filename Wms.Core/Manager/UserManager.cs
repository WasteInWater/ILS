...
namespace Wms.Core
{
    ...
    public class UserManager : IUserManager,IScoped
    {
        ...
        public long UserId
        {
            get => long.Parse(_httpContextAccessor.HttpContext.User.FindFirst(ClaimConst.Claim_UserId)?.Value);
        }

        public string Account
        {
            get => _httpContextAccessor.HttpContext.User.FindFirst(ClaimConst.Claim_Account)?.Value;
        }

        public string Name
        {
            get => _httpContextAccessor.HttpContext.User.FindFirst(ClaimConst.Claim_Name)?.Value;
        }

        public bool SuperAdmin
        {
            get => _httpContextAccessor.HttpContext.User.FindFirst(ClaimConst.Claim_SuperAdmin)?.Value == ((int)AdminType.SuperAdmin).ToString();
        }
