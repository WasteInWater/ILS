...
namespace Wms.Core.Service
{
    ...
    public class SysUserService : ISysUserService, IDynamicApiController, ITransient
    {
        ...
        public async Task<dynamic> QueryUserPageList([FromQuery] UserInput input)
        {
            ...
            var Ischeck = !string.IsNullOrEmpty(input.Name?.Trim());
            ...
            var users = await _sysUserRep.DetachedEntities
                                         ...
                                         .Where((Ischeck, u=>EF.Functions.Like(u.n.u.Name,$"%{input.Name}%")),
                                         (!string.IsNullOrEmpty(input.Account?.Trim()), m=>EF.Functions.Like(m.n.u.Account,$"%{input.Account}%")))
            ...
            return PageResult<UserOutput>.LayuiPageResult(users);
        }
