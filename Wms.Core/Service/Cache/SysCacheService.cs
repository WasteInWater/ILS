...
namespace Wms.Core.Service
{
    ...
    public class SysCacheService : ISysCacheService, IDynamicApiController, ISingleton
    {
        ...
        /// <returns></returns>
        public async Task<List<LayuiMenu>> GetMenu(long userId, string appCode)
        {
            var cacheKey = CommonConst.CACHE_KEY_MENU + $"{userId}-{appCode}";
            return await _cache.GetAsync<List<LayuiMenu>>(cacheKey);
        }
        ...
        /// <returns></returns>
        [NonAction]
        public async Task SetMenu(long userId, string appCode, List<LayuiMenu> menus)
        {
            var cacheKey = CommonConst.CACHE_KEY_MENU + $"{userId}-{appCode}";
